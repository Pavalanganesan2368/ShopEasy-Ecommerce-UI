  ...
  if (featuredContainer) {
    ...
    featuredProducts.forEach((product) => {
      ...
      productElement.className =
        "product-card border border-gray-200 rounded-md p-2";
      productElement.innerHTML = `
                <img src = "${product.image}" alt = "${product.name}" class = "w-full h-100 rounded-md">
                <div>
                    <section class="flex justify-between py-4 text-xl font-bold">
                        <h3 class = "product-title">${product.name}</h3>
                        <p class = "product-price">$${product.price.toFixed(2)}</p>
                    </section>
                    <button class = "add-to-cart w-full bg-sky-500 p-2 rounded-md font-bold text-white" data-id = "${product.id}">ADD TO CART</button>
                </div>
            `;
